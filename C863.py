...
class C863RS232CorruptionException(Exception):
  def __init__(self, c):
    super(SMC100RS232CorruptionException, self).__init__('RS232 corruption detected: %s'%(hex(ord(c))))

class C863InvalidResponseException(Exception):
...
class C863(object):
  ...
  def __init__(self, smcID, port, backlash_compensation=True, silent=True, sleepfunc=None):
    ...
    print 'Connecting to C863 on %s'%(port)

    self._port = serial.Serial(
        port = port,
        baudrate = 38400,   #corrected baudrate
        bytesize = 8,
        stopbits = 1,
        parity = 'N',
        xonxoff = True,
...
#4 ON ON OFF OFF
  ...
  def move_relative_mm(self, dist_mm, waitStop=False):
    """
  ...
  def move_absolute_mm(self, position_mm, waitStop=False):
    """
